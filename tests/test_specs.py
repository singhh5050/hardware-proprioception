...
from hwprop.specs import get_hardware_specs, get_model_configs, GB, TFLOPS
...
class TestModelConfigs:
    ...
    def test_llama31_70b_param_count(self):
        ...
        assert abs(m.num_params - 70e9) / 70e9 < 0.10, (
    ...
    def test_llama31_8b_kv_bytes_per_token(self):
        ...
        assert m.kv_bytes_per_token == expected, (
            f"Expected {expected}, got {m.kv_bytes_per_token}"
        )
    ...
    def test_qwen25_72b_param_count(self):
        ...
        assert abs(m.num_params - 72e9) / 72e9 < 0.10, (
            f"Qwen2.5 72B params: {m.num_params / 1e9:.2f}B"
        )

    def test_phi4_param_count(self):
        m = get_model_configs()["Phi-4-14B"]
        assert abs(m.num_params - 14e9) / 14e9 < 0.10, (
            f"Phi-4 params: {m.num_params / 1e9:.2f}B"
        )

    def test_gemma3_27b_explicit_head_dim(self):
        m = get_model_configs()["Gemma-3-27B"]
        assert m._head_dim == 128
        assert m.d_model // m.num_heads == 168

    def test_gemma3_27b_param_count(self):
        m = get_model_configs()["Gemma-3-27B"]
        assert abs(m.num_params - 27e9) / 27e9 < 0.15, (
            f"Gemma-3 27B params: {m.num_params / 1e9:.2f}B"
        )

    def test_gemma3_4b_explicit_head_dim(self):
        m = get_model_configs()["Gemma-3-4B"]
        assert m._head_dim == 256
        assert m.d_model // m.num_heads == 320  # would be wrong without explicit

    def test_llama32_3b_param_count(self):
        m = get_model_configs()["LLaMA-3.2-3B"]
        assert abs(m.num_params - 3.2e9) / 3.2e9 < 0.10, (
            f"LLaMA-3.2 3B params: {m.num_params / 1e9:.2f}B"
        )

    def test_qwen3_32b_param_count(self):
        m = get_model_configs()["Qwen3-32B"]
        assert abs(m.num_params - 32e9) / 32e9 < 0.15, (
            f"Qwen3 32B params: {m.num_params / 1e9:.2f}B"
        )

    def test_phi4_mini_param_count(self):
        m = get_model_configs()["Phi-4-mini-3.8B"]
        assert abs(m.num_params - 3.8e9) / 3.8e9 < 0.15, (
            f"Phi-4-mini params: {m.num_params / 1e9:.2f}B"
        )

    def test_llama4_scout_is_moe(self):
        m = get_model_configs()["Llama-4-Scout-17B-16E"]
        assert m.is_moe
        assert m.num_experts == 16
        assert m.experts_per_token == 1
        # Total params should be ~100-110B (close to 109B announced)
        assert 80e9 < m.num_params < 130e9, (
            f"Scout total params: {m.num_params / 1e9:.1f}B"
        )

    def test_deepseek_v3_is_moe(self):
        m = get_model_configs()["DeepSeek-V3"]
        assert m.is_moe
        assert m.num_experts == 256
        assert m.experts_per_token == 8
        # Total params ~671B; our formula slightly overestimates
        assert 500e9 < m.num_params < 800e9, (
            f"DeepSeek-V3 total params: {m.num_params / 1e9:.1f}B"
        )

    def test_dense_models_not_moe(self):
        dense_names = ["LLaMA-3.1-8B", "Qwen2.5-72B", "Gemma-3-27B", "Phi-4-14B"]
        for name in dense_names:
            m = get_model_configs()[name]
            assert not m.is_moe, f"{name} should not be MoE"
            assert m.num_experts == 1
